...
def get_response(
    ...
    with retrieve_proxy():
        response = requests.post(
            shared.state.completion_url, headers=headers, json=payload, stream=True, timeout=timeout
        )
...
def stream_predict(
    ...
    if len(all_token_counts) == 0:
        ...
        user_token_count = input_token_count + system_prompt_token_count
    else:
    ...
    try:
        response = get_response(
            openai_api_key, system_prompt, history, temperature, top_p, True, selected_model
        )
...
def predict_all(
    ...
    try:
        response = get_response(
            openai_api_key, system_prompt, history, temperature, top_p, False, selected_model
        )
    except requests.exceptions.ConnectTimeout:
        status_text = standard_error_msg + connection_timeout_prompt + error_retrieve_prompt
        return chatbot, history, status_text, all_token_counts
...
def predict(
    ...
    if files:
        ...
        with retrieve_proxy():
            ...
            prompt_helper = PromptHelper(
                max_input_size=4096, num_output=5, max_chunk_overlap=20, chunk_size_limit=600
            )
    ...
    if len(history) > 1 and history[-1]["content"] != inputs:
        logging.info(
            "回答为：" + colorama.Fore.BLUE + f"{history[-1]['content']}" + colorama.Style.RESET_ALL
        )
